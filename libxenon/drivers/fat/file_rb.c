...
static int open_internal(int volume, const char* pathname, int flags, bool use_cache)
{
    RB_DIR* dir;
...
# endif
    ...
    if ( name ) {
        *name = 0; 
        dir = rb_opendir(volume, pathnamecopy);
        *name = '/';
        name++;
    }
    else {
        dir = rb_opendir(volume, "/");
        name = pathnamecopy+1;
    }
    if (!dir) {
    ...
    while ((entry = rb_readdir(dir))) {
        if ( !rb_strcasecmp(name, entry->d_name) ) {
...
int rb_open(int volume, const char* pathname, int flags)
{
    /* By default, use the dircache if available. */
    return open_internal(volume, pathname, flags, true);
}
...
int rb_remove(int volume, const char* name)
{
    int rc;
    struct filedesc* file;
    /* Can't use dircache now, because we need to access the fat structures. */
    int fd = open_internal(volume, name, O_WRONLY, false);
    if ( fd < 0 )
        return fd * 10 - 1;
...
int rb_rename(int volume, const char* path, const char* newpath)
{
    int rc, fd;
    RB_DIR* dir;
    char* nameptr;
    ...
    fd = rb_open(volume, newpath, O_RDONLY);
    ...
    fd = open_internal(volume, path, O_RDONLY, false);
    if ( fd < 0 ) {
        errno = EIO;
        return fd * 10 - 2;
    }
...
int rb_ftruncate(int fd, off_t size)
{
    int rc, sector;
    struct filedesc* file = &openfiles[fd];
...
static int readwrite(int fd, void* buf, long count, bool write)
{
    ...
    if ( file->cacheoffset != -1 ) {
        ...
        int headbytes = (count>SECTOR_SIZE - offs)?SECTOR_SIZE - offs:count;// MIN(count, SECTOR_SIZE - offs);
...
ssize_t rb_write(int fd, const void* buf, size_t count)
{
...
ssize_t rb_read(int fd, void* buf, size_t count)
{
    return readwrite(fd, buf, count, false);
}


off_t rb_lseek(int fd, off_t offset, int whence)
{
    off_t pos;
    long newsector;
    long oldsector;
