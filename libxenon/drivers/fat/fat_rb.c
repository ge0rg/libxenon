                         ...
                         bool dotdir)
{
    ...
    if(dotdir) {
        int i;
        rb_strlcpy(shortname, name, 12);
        for(i = rb_strlen(shortname); i < 12; i++)
            shortname[i] = ' ';

        entries_needed = 1;
    } else {
        create_dos_name((unsigned char *)name, shortname);
        ...
        for (i = 0; i < DIR_ENTRIES_PER_SECTOR; i++)
        {
            switch (buf[i * DIR_ENTRY_SIZE]) {
              ...
              default:
                ...
                if (!rb_strncmp(shortname, buf + i * DIR_ENTRY_SIZE, 11)) {
...
static void create_dos_name(const unsigned char *name, unsigned char *newname)
{
    ...
    /* needs to randomize? */
    if((ext && (rb_strlen(ext) > 4)) ||
       ((ext ? (unsigned int)(ext-name) : rb_strlen(name)) > 8) )
        randomize = 1;
...
static void randomize_dos_name(unsigned char *name)
{
    ...
    rb_snprintf(num, 9, "~%d", cnt);   /* allow room for trailing zero */
    numlen = rb_strlen(num);           /* required space */
            ...
            if ( ( dir->sectorcache[entrypos + FATDIR_ATTR] &
                ...
                if ( parse_direntry(entry, dir->sectorcache + entrypos) ) {
                    ...
                    if (nb_longs != 0 && last_long_ord == 1) {
                        ...
                        rb_strcpy(shortname, entry->name);
                        entry->name[0] = 0;
                        ...
                        for (j = 0; j < nb_longs * FATLONG_NAME_BYTES_PER_ENTRY; j += 2) {
                            ...
                            if (longname_utf8len + segm_utf8len >= FAT_FILENAME_BYTES) {
                                ...
                                rb_strcat(entry->name + longname_utf8len, longname_utf8segm);
                        ...
                        if (longname_utf8len >= FAT_FILENAME_BYTES) {
                            ...
                            *utf8 = 0;
							*/
							rb_strcpy(entry->name,shortname);
